    ...
    let cmd_cd = vorpal.command('cd [location]', 'Change location')
        ...
        .action(function (args, cb) {
            ...
            locations.findOne({
                ...
                vorpal.delimiter(chalk.magenta(`${config.prompt}:${data.name}>`));
        ...
        .action(function (args, cb) {
            if (!args.name) {
                ...
                locations.findOne({
                    ...
                    this.log('name:', data.name, '(', data._id, ') url:', data.url || '<default>', 'username:', data.username || '<default>', 'password:', data.password || '<default>', 'deviceId:', data.deviceId || '<none>');
                    cb();
                });
        });
